package test4.partition.spouts;

import java.util.List;
import java.util.Map;

import test4.demo.spouts.TransactionMetadata;
...
public class PartitionedTransactionalSpout extends
		BasePartitionedTransactionalSpout<TransactionMetadata> {
	private static final long serialVersionUID = 1L;
	public static final int MAX_TRANSACTION_SIZE = 30;

	public static class TweetsPartitionedTransactionalCoordinator implements
			Coordinator {
		@Override
	...
	public static class PartitionedTransactionalEmitter implements
			Emitter<TransactionMetadata> {
		RQ rq = new RQ();
		...
		public void emitPartitionBatch(TransactionAttempt tx,
				BatchOutputCollector collector, int partition,
				TransactionMetadata partitionMeta) {
			if (partitionMeta.quantity <= 0)
				return;

			List<String> messages = rq.getMessages(partition,
					partitionMeta.from, partitionMeta.quantity);
			long Id = partitionMeta.from;
			for (String msg : messages) {
				collector.emit(new Values(tx, "" + Id, msg));
				Id++;
			}
		}
	...
	public Coordinator getCoordinator(Map conf, TopologyContext context) {
		return new PartitionedTransactionalCoordinator();
	}

	@SuppressWarnings("rawtypes")
	@Override
	public Emitter<TransactionMetadata> getEmitter(Map conf,
			TopologyContext context) {
		return new PartitionedTransactionalEmitter();
	}

	@Override
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declare(new Fields("txid", "header_id", "field", "value"));
	}
}
